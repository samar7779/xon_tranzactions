...
/**
 * Diagnostika paneli — CRM client kaliti xonadonlar/obyektlar ro'yxatini
 * beradimi (shaxmatka g'oyasi uchun). Bir tugma: bosiladi, natija shu yerda.
 * FAQAT O'QISH — CRM'da hech narsa o'zgarmaydi.
 */
function InventoryProbePanel() {
  const [open, setOpen] = useState(false);
  ...
  const [res, setRes] = useState<{
    ...
    base?: string;
    working?: string[];
    tried?: Array<{ path: string; method: string; status: any; ok: boolean; count: number | null; keys: string[]; sample: any }>;
    error?: string;
  } | null>(null);
  ...
  const run = async () => {
    ...
    setRes(null);
    try {
      const r = await api.get<any>('/crm/inventory-probe', { timeout: 180_000 });
      setRes(r);
      const n = r?.working?.length || 0;
      if (n > 0) toast.success(`${n} ta endpoint ishladi ✓`);
      else toast('Hech qaysi endpoint xonadonlar ro\'yxatini bermadi', { description: 'Admin API kerak bo\'ladi' });
    } catch (e: any) {
      setRes({ ok: false, error: e?.message || 'Xato' });
  ...
  return (
    <Card className="border-0 shadow-soft">
      <CardContent className="p-0">
        <button
          onClick={() => setOpen((v) => !v)}
          className="w-full flex items-center gap-3 px-5 py-3.5 text-left hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
        >
          <span className="w-9 h-9 rounded-xl bg-gradient-to-br from-sky-500 to-cyan-600 grid place-items-center text-white shadow-md shrink-0">
            <Building2 className="h-4 w-4" />
          </span>
          <span className="flex-1 min-w-0">
            <span className="block text-[13px] font-bold text-slate-800 dark:text-slate-100">
              Xonadonlar ro&apos;yxati tekshiruvi
            </span>
            <span className="block text-[11px] text-slate-500 dark:text-slate-400">
              CRM kaliti bo&apos;sh xonadonlarni beradimi — shaxmatka uchun (faqat o&apos;qish)
            </span>
          </span>
        ...
        {open && (
          <div className="px-5 pb-5 border-t border-slate-100 dark:border-slate-800 pt-4 space-y-3">
            <Button onClick={run} disabled={running} className="h-10 rounded-xl gap-2">
              {running ? <><Loader2 className="h-4 w-4 animate-spin" /> Tekshirilmoqda… (1-2 daqiqa)</> : <>Tekshirishni boshlash</>}
            </Button>

            {res && (
              <div className="space-y-3">
                {res.error ? (
                  <div className="rounded-xl bg-rose-50 dark:bg-rose-950/30 ring-1 ring-rose-200 dark:ring-rose-900 px-4 py-3 text-[12px] text-rose-700 dark:text-rose-300">
                    {res.error}
                  </div>
                ) : (
                  <>
                    <div className={cn(
                      'rounded-xl px-4 py-3 text-[12.5px] font-semibold ring-1',
                      (res.working?.length || 0) > 0
                      ...
                      {(res.working?.length || 0) > 0
                        ? <>✓ Ishladi: {res.working!.join(', ')}</>
                        : <>Bizdagi kalit xonadonlar ro&apos;yxatini bermadi — XonSaroy&apos;dan admin API so&apos;rash kerak</>}
                    </div>
                    ...
                    <div className="rounded-xl ring-1 ring-slate-200 dark:ring-slate-700 overflow-hidden">
                      <table className="w-full text-[11.5px]">
                        <thead className="bg-slate-50 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400">
                          <tr>
                            <th className="text-left px-3 py-2 font-semibold">Yo&apos;l</th>
                            <th className="text-left px-3 py-2 font-semibold">Usul</th>
                            <th className="text-left px-3 py-2 font-semibold">Javob</th>
                            <th className="text-left px-3 py-2 font-semibold">Qator</th>
                            <th className="text-left px-3 py-2 font-semibold">Maydonlar</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                          {(res.tried || []).map((t, i) => (
                            <tr key={i} className={t.ok ? 'bg-emerald-50/60 dark:bg-emerald-950/20' : ''}>
                              <td className="px-3 py-1.5 font-mono">{t.path}</td>
                              <td className="px-3 py-1.5 text-slate-500">{t.method}</td>
                              <td className="px-3 py-1.5 tabular-nums">{String(t.status)}</td>
                              <td className="px-3 py-1.5 tabular-nums">{t.count ?? '—'}</td>
                              <td className="px-3 py-1.5 text-slate-500 truncate max-w-[320px]">
                                {t.keys?.length ? t.keys.slice(0, 8).join(', ') : '—'}
                              </td>
                            </tr>
                          ))}
                        </tbody>
