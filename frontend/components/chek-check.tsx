...
// Chek order — "Tekshirish" UI (umumiy komponent).
// Asosiy web sahifasidagi tekshirish bilan AYNAN bir xil — Telegram Mini App
// sahifasi ham shuni ochadi (responsive: PC → 2 ustun, telefon → 1 ustun).
...
import {
  ...
  Hash, Search, Image as ImageIcon, ScanLine, RotateCcw,
  Building2, CalendarDays, Coins, FileSignature, Landmark, FileText,
  ZoomIn, X, ScrollText, ArrowRightLeft, Home, ListChecks, User2, Tag, ChevronRight,
} from 'lucide-react';
import { Card } from '@/components/ui/card';
import { api } from '@/lib/api';
import { cn, formatMoney } from '@/lib/utils';

export type Cond = { order: boolean | null; account: boolean | null; date: boolean | null; amount: boolean | null; contract: boolean | null };
type Extracted = {
...
export const RESULT_META: Record<string, { cls: string; Icon: any; glow: string }> = {
...
export const resultLabel = (tr: any, k: string) => tr(`result.${RESULT_KEY[k] || 'notFound'}`);
...
/**
 * Tekshirish UI. `guest` — Telegram mehmoni (chuqur modallar yashiriladi).
 * `onResultsChange` — natijalar o'zgarganda (AI yordamchi konteksti uchun).
 * `onChecked` — tekshiruvdan keyin (tarixni yangilash uchun).
 */
export function ChekCheck({ guest = false, onResultsChange, onChecked }: {
  guest?: boolean;
  onResultsChange?: (r: OrderResult[] | null) => void;
  onChecked?: () => void;
}) {
  ...
  useEffect(() => { onResultsChange?.(results); /* eslint-disable-next-line */ }, [results]);
  ...
  const analyzeMut = useMutation({
    ...
    onSuccess: (r) => { setResults(r.results); onChecked?.(); toast.success(tr('toast.docsChecked', { n: r.results.length })); },
  ...
  const manualMut = useMutation({
    ...
    onSuccess: (r) => { setResults(r.results); onChecked?.(); toast.success(tr('toast.ordersChecked', { n: r.results.length })); },
    onError: (e: any) => toast.error(e?.message || tr('toast.error')),
  });
  ...
  const pickContract = (c: string) => { setSelectedContract(c); setContractQ(c); setSuggFocused(false); setResults(null); };
  ...
  return (
    <div className="space-y-4">
      ...
      {(preview || results) && (
        <div className="flex justify-end">
          <button onClick={reset}
            className="group inline-flex items-center gap-1.5 px-3.5 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 text-white text-[12px] font-semibold shadow-md shadow-indigo-500/25 hover:shadow-lg hover:shadow-indigo-500/40 hover:-translate-y-0.5 transition-all">
        ...
        <div className="space-y-3 min-w-0">
          {busy && !results ? (
            ...
            <>
              ...
              {results[activeResult] && <ResultCard r={results[activeResult]} guest={guest} />}
            </>
          ) : (
      ...
      {panelContract && <ContractInfoPanel contract={panelContract} />}
...
// ─── Natija kartasi ───
function ResultCard({ r, guest }: { r: OrderResult; guest?: boolean }) {
  const tr = useTranslations('chekOrder');
...
// SHARTLAR ustuni — bitta icon + popover (Tarix jadvalida ham ishlatiladi)
export function CondCell({ c }: { c: Cond }) {
  const tr = useTranslations('chekOrder');
